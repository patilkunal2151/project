...
public ServiceRequest getByVendorId(int VendorId) {
		
		Optional<ServiceRequest> request = serviceRequestDao.findByVendorId(VendorId);
		if(request.isPresent())
		{
			return request.get(); 			
		}else
	...
	public ServiceRequest getByCustomerIdAndStatus(int customer_id, String status) {				
		...
		if(request.isPresent())
		{
			request.get().setStatus("PENDING");
			serviceRequestDao.save(request.get());
			return request.get();
		}else
		{
			return null;
...
public List<ServiceRequest> getAllServicesByVendorId(int customer_id) {
		
		List<ServiceRequest> list = null;
		list = serviceRequestDao.findAllByVendorId(customer_id); 	
		if(list != null)
		{
			return list; 
