...
// ---------------------------------------------------------------------------
// ________________________________admin signup________________________________
// ---------------------------------------------------------------------------
	...
	public ResponseEntity<?> adminSignup(@RequestBody Admin newAdmin) throws AuthenticationException {
		...
		if (userService.addUser(new User(newAdmin.getEmail(), newAdmin.getPassword(), "ADMIN", 1)) != null
				&& adminService.addAdmin(newAdmin) != null) {
...
// ---------------------------------------------------------------------------
// ________________________________Sign in Admin________________________________
// ---------------------------------------------------------------------------
	@PostMapping("/signin")
	public ResponseEntity<?> signInVendor(@RequestBody LoginCredentials admin) {
		ResponseEntity<?> resp = null;
		Map<String, Object> map = new HashMap<String, Object>();

		System.out.println("in Admin signIn");
		try {
			Admin signin = adminService.findAdminByEmailAndPassword(admin);
			if (signin != null) {
				map.put("status", "success");
				map.put("data", signin);
				resp = new ResponseEntity<>(map, HttpStatus.OK);
...
// ---------------------------------------------------------------------------
// ________________________________Forgot password Admin________________________________
// ---------------------------------------------------------------------------
	...
	public ResponseEntity<?> forgotPassword(@RequestBody Admin admin) {
		...
		try {
			boolean signin = adminService.resetPassword(admin);
			if (signin == true) {
				map.put("status", "success");
...
// ---------------------------------------------------------------------------
	...
	public ResponseEntity<?> VendorSignup(@RequestBody Vendor newVendor) throws AuthenticationException {
		...
		if (userService.addUser(new User(newVendor.getEmail(), newVendor.getPassword(), "VENDOR", 1)) != null
	...
	public ResponseEntity<?> updateVendor(@PathVariable(value = "id") int vendor_Id,
		...
		if (vendorService.addVendor(vendorDetails) != null) {
			map.put("status", "success");
			resp = new ResponseEntity<>(map, HttpStatus.OK);
...
// ---------------------------------------------------------------------------
	...
	@PutMapping("/giveAnswer/{id}")
	public ResponseEntity<?> updateQuestion(@PathVariable(value = "id") int question_id,
			@Valid @RequestBody Question questionDetails) throws Exception {
		ResponseEntity<?> resp = null;
		Map<String, Object> map = new HashMap<String, Object>();

		questionDetails.setQuestion_id(question_id);

		if (questionDao.save(questionDetails) != null) {
			map.put("status", "success");
			resp = new ResponseEntity<>(map, HttpStatus.OK);
...
// ---------------------------------------------------------------------------
// ____________________________________Delete offer
// ---------------------------------------------------------------------------
