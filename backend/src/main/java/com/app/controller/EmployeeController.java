...
// -------------------------------------------------------------------------------------------------------
// ________________________________Sign in Employee________________________________
// -------------------------------------------------------------------------------------------------------
	@PostMapping("/signin")
	public ResponseEntity<?> signInEmployee(@RequestBody LoginCredentials emp) {
		...
		System.out.println("in Employee signIn");
		try {
			Employee signin = empService.findEmployeeByEmailAndPassword(emp);
			if (signin != null) {
				map.put("status", "success");
				map.put("data", signin);
				resp = new ResponseEntity<>(map, HttpStatus.OK);
...
// ---------------------------------------------------------------------------
// ________________________________Forgot password Employee
// ---------------------------------------------------------------------------
			...
			public ResponseEntity<?> forgotPassword(@RequestBody Employee emp) {
				...
				System.out.println("in Employee reset password");
				try {
					boolean signin = empService.resetPassword(emp);
					if (signin == true) {
						map.put("status", "success");
...
// -----------------------------------------------------------------------------------------------
// ________________________________Update Profile________________________________
// -----------------------------------------------------------------------------------------------
	...
	public ResponseEntity<?> updateEmployee(@PathVariable(value = "id") int employee_id,
		...
		if (empService.addEmployee(employeeDetails) != null) {
			map.put("status", "success");
			resp = new ResponseEntity<>(map, HttpStatus.OK);
...
// ---------------------------------------------------------------------------
	...
	public ResponseEntity<?> customerSignup(@RequestBody Customer newCustomer) throws AuthenticationException {
		...
		if (userService.addUser(new User(newCustomer.getEmail(), newCustomer.getPassword(), "CUSTOMER", 1)) != null
...
// ----------------------------------------------------------------------------
		...
		public ResponseEntity<?> fetchAllEmployee() {
			...
			System.out.println("in fetch all Employee");

			try {
				List<Employee> customer = employeeDao.findAll();
				map.put("status", "success");
				map.put("data", customer);
				resp = new ResponseEntity<>(map, HttpStatus.OK);
			} catch (Exception e) {
...
// ------------------------------------------------------------------
	...
	public ResponseEntity<?> makeInvoiceFromServiceRequest(@PathVariable int customer_id,
		...
		if (request != null) {
			...
			double discount = (100 - serviceRequest.getDiscount()) / 100;
...
//--------------------------------------------------------------------------------------------------
	...
	// ------------------------------------------------------------------------------------------------
	//____________________________________ Get stock details by id
	// --------------------------------------------------------------------------------------------------
